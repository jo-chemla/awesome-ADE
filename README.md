...
A comparison of agentic development environments. The daily section is intentionally **activity-only**: a GitHub leaderboard tracking stars, contributor counts, and recency.
...
The leaderboard is regenerated daily from the GitHub API. Historical snapshots are kept in [`stats/stats-history.json`](./stats/stats-history.json).

## Feature matrix

The feature matrix is static, maintained through the individual files in [`content/tools/`](./content/tools), and deliberately kept separate from the activity leaderboard.

<details>
<summary>Show feature matrix</summary>
